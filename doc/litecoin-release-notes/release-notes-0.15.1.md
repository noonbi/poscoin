Poscoin Core version *0.15.1* is now available from:

  <https://download.poscoin.org/poscoin-0.15.1/>
...
A number of changes to the way Poscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 blessings per kB (0.00001 POS). This is to prepare for dropping the minimum transaction fee to 0.1 blessings per kB (0.0001 POS) in the next release.
